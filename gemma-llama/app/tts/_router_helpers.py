...
include both the success-shape fields (``audio_url`` / ``audio_duration_ms`` /
``voice``) and an optional ``audio_error`` so the caller never has to branch
on engine availability.
"""
...
from .integration import attach_file_tts, wrap_stream_with_tts
